// Homepage English Language Pack
export default {
  // 导航栏
  'nav.title': 'Library Seat Reservation',

  // Search Area
  'search.placeholder': 'Enter search keywords',
  'search.tags.power': 'With Power Socket',
  'search.tags.window': 'Near Window',
  'search.tags.single': 'Single Desk',
  'search.tags.double': 'Double Desk',
  'search.tags.group': 'Group Discussion Table',

  // Action Cards
  'actions.myReservation': 'My Reservations',
  'actions.renew': 'Quick Renewal',
  'actions.checkin': 'Scan to Check-in',
  'actions.checkout': 'Scan to Check-out',
  // Opening Hours
  'openingHours.title': "Today's Opening Hours",
  'openingHours.time': '08:00 - 22:00',
  'openingHours.status': 'Open',

  // Seat Status Overview
  'seatStatus.title': 'Seat Status Overview',
  'seatStatus.total': 'Total Seats',
  'seatStatus.available': 'Available Seats',
  'seatStatus.reserved': 'Reserved Seats',
  'seatStatus.maintenance': 'Maintenance Seats',

  // Real-time Information
  'realTime.title': 'Real-time Information',
  'realTime.users': 'Users in Library',
  'realTime.users.sublabel': 'Current Online',
  'realTime.comfort': 'Comfortable Reading Environment',
  'realTime.comfort.sublabel': 'Library Temperature',
  'realTime.quiet': 'Quiet Study Environment',
  'realTime.quiet.sublabel': 'Noise Level',
  'realTime.popular': 'Most Popular Area',
  'realTime.popular.sublabel': 'Popular Area',
  'realTime.booked': 'Booked Seats Today',
  'realTime.booked.sublabel': 'Current Bookings',
  'realTime.total': 'Total Bookings Today',
  'realTime.total.sublabel': "Today's Total",

  // Recommended Seats
  'recommend.title': 'Recommended Seats',

  // Seat Items
  'seat.window': 'Window Seat',
  'seat.quiet': 'Quiet Seat',
  'seat.power': 'Power Socket Seat',
  'seat.type.single': 'Single Desk',
  'seat.type.double': 'Double Desk',

  // Activity Notification
  'activity.title': 'Activity Notification',
  'activity.viewMore': 'More Activities',

  // Activity Cards
  'activity.marathon': 'Reading Marathon Challenge',
  'activity.marathon.desc': 'Challenge 7 days of continuous reading, win exquisite gifts!',

  'activity.sharing': 'Book Sharing Session',
  'activity.sharing.desc': 'Monthly sharing of your reading thoughts and insights.',

  'activity.retrieval': 'Literature Retrieval Training',
  'activity.retrieval.desc': 'Learn efficient literature retrieval skills.',

  'activity.classic': 'Classic Recitation Activity',
  'activity.classic.desc': 'Recite Chinese classics, inherit excellent culture.',
};
